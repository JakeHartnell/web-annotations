EpubAnnotations.ReflowableAnnotations = Backbone.Model.extend({
    ...
    initialize : function (attributes, options) {
        ...
        //Begin IE workaround:
        // When we want to get an iframe document reference
        // and you try to use it in IE you might get a "Permission denied" exception
        // if the callstack begins with a user action such as clicking a button or selecting text
        this.getFromModel = this.get;
        this.get = function(attr){
            var value = this.getFromModel(attr);
            if(attr == "contentDocumentDOM"){
                try{
                    if (value === undefined) {
                        console.log('IE contentDocument exception: empty value');
                    }
                } catch (ex) {
                    console.log('IE contentDocument exception caught:');
                    console.log(ex);
                    value = $('.'+this.getFromModel('contentDocumentFrameUid'))[0].contentDocument;
                }
            }
            return value;
        };

        //Register iframe with a unique id for use in an IE issue workaround
        var $contentDocumentFrame = $(this.get("contentDocumentFrame"));
        var frameUid = EpubAnnotations.Helpers.generateShortUid();
        $contentDocumentFrame.addClass(frameUid);
        this.set("contentDocumentFrameUid",frameUid);
        //End IE workaround
